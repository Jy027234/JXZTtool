...
2026-07-14 验收推进：已新增固定样本清单和 `tools/p0_quality_audit.py`，对 `D:\app\uploads` 的 7 类真实样本执行 `projection=full`，产出 `coverage_report.jsonl`、逐样本 full 工件和 chunk→block→page 追溯统计。按当前代码重跑的最新工件为 7/7 样本完成、31 个页级记录、1,919 blocks、746 chunks，746/746 个 chunk 可追溯，缺口原因完整率 100%；2 个无文本/图片的空白页现显式记为 `page_without_extractable_content`。新增只读 `--embedding-provider fake` 复核，同一批样本 7/7 的 embedding coverage 通过，证明本地解析→chunk→embedding 链路可复现，但不替代真实 embedding/RAG 验收。随后在项目 `.venv` 安装 `pymupdf4llm 0.3.4` 和 Docling 2.113.0，对 3 个 PDF 页段完成 `pdf-text`/`pymupdf4llm-local` 对标，并在 50 页受控集完成 100 次 provider runs（候选 50/50、基线 49/50）。用户明确授权后，`tools/ai_gold_review.py` 对 50/50 页面完成 AI-assisted review（范围明确为 `ai_assisted_review_not_human_gold`），交叉核对截图、文本探针、哈希和 `pdf-text` 基线证据；这不等同于独立人工签字。对比工具随后改为默认关闭 `tracemalloc`，避免 native PDF 解析被 Python 内存追踪放大；真实耗时复评中候选平均 0.272 s、p95 0.561 s、最大 5.818 s，基线已完成页平均 2.027 s。候选在扫描/表格/多栏页面上更快，但 block/table/figure 数量差异明显，普通 PDF 第 165 页仍为最高长尾（5.818 s vs 2.567 s）。`provider_gold_evaluation` 现额外输出 `gold_evaluation.risk_summary`，按文档和页码给出耗时长尾、基线缺失及结构变化的复核优先级；`pymupdf4llm-local` 也已支持受控透传 `ignore_graphics / graphics_limit / table_strategy` 等离线调参，默认行为不变。普通 PDF 的 `ignore_graphics=true` 临时复跑已证明耗时可降至平均 0.276 s，但表格数降为 0，因此未采纳。默认 `pdf-text` 路径在最新代码下完成 3 次连续复测：耗时 136.926/120.755/135.035 s，中位数 135.035 s；三次均无错误，2,035 blocks、2,035 chunks、44 tables 保持一致。相对最初 148.144 s 基线，中位数下降约 8.85%，但耗时波动较大，仅作为稳定性观察值，不写成硬 SLA。候选仍因许可证、结构风险和长尾保持 `remain_shadow_only`，不自动改路由；如需旧的 Python 峰值内存指标，Provider 对比命令可显式追加 `--track-python-memory`，但该模式不用于真实耗时 SLA。Docling 已完成 3×5 页窗口运行，但中文安装路径的原生 glyph 资源问题刚由 ASCII junction/copy fallback 修复；254 页整文档探针约 15 分钟未完成，故仍保持 `evaluate/pending`，MinerU skipped。`tools/provider_gold_evidence.py` 已为 50 个页面渲染原始截图、文本探针和哈希证据，当前 packet 状态为 `approved=50 / pending=0 / rejected=0`；候选窗口另外由 `tools/provider_stability_gate.py` 完成 3 次连续双 Provider 报告，9/9 运行完成且候选 quality signature 稳定，baseline 结构差异保留为 warning。若发布治理要求独立人工 gold，仍需补做人工签字，不能把 AI-assisted 批准冒充人工验收。详见 [P0 验收记录](D:\个人文件\个人开发\解析管理中台\docs\p0-acceptance-20260714.md)。

随后补做的 `docling-local` 50 页全 Gold 评估（`var/self-check/provider-gold-docling-20260715-r1.json`）全部完成，但平均 Gold 分数 `53.021`、平均耗时 `11.444 s`、最大 `234.983 s`，39/50 页有 critical token 缺失，继续保持 `evaluate/pending`。本地 Transformer embedding 也新增独立端到端 acceptance（`var/self-check/local-rag-acceptance-20260715.json`）：6/6 chunks embedded、manifest coverage `1.0`、4/4 查询 hit@3=`1.0`。readiness 现在按 `p0-core`（默认本地路由）和 `production`（候选 route promotion/远程 embedding）分 scope；`p0-core` 工件已通过，生产扩展的许可证和真实凭证仍作为独立外部门禁，不会误阻断默认 `pdf-text`。

本轮补充了 Docling 的候选 `reuse_converter` 开关：同一长生命周期 parser 可复用已构造的 `DocumentConverter`，适合分片重试/重复解析；10 页受控 cold/warm 探针为 `29.883 s → 13.299 s`（约 `-55.5%`），结构指纹保持一致，详见 `var/self-check/docling-reuse-probe-r19.json`。首块 provenance 同时记录 `converter_reuse_enabled / converter_cache_hit`，方便把冷启动与稳态耗时拆开。默认库行为仍关闭，且未据此改变 `pdf-text` 路由；`fast-text` 与 converter 复用必须分别做全量、工作集和并发安全对比后才能进入准入门禁。

Provider suite 失败项现在会补 `failure_category`，并在 gate summary 汇总 `provider_unavailable / timeout / invalid_input / permission_denied / unsupported / provider_failed`，用于区分可选依赖或输入环境阻塞与真正的 Provider 运行退化；该分类只影响诊断，不自动改 route。

若 Provider 暴露 Docling converter telemetry，对比工件还会写入 `converter_cache_state`，并在 suite summary 汇总 `converter_cache.observations / hits / misses`；当前 CLI 每个样本仍按独立 parser 测量，不能把单次 cold run 冒充 warm SLA。

对比 CLI 的 `--reuse-parser-instances` 仅用于候选 warm-state probe：同一 Provider 在 suite 内复用一个 parser 实例，默认值为 `false`，不会改变生产 `pdf-text` 路由或既有冷启动基线。

`pdf-text` 的 `post_process.parse_cache` 现也会在对比工件中记录 `parse_cache_state`，并汇总 `parse_cache.observations / hits / misses`；该缓存只作用于同源、同租户和同请求选项的重解析，返回时会重新绑定文档与 Block 标识。
...
与此同时，`tools/self_check.py` 的 Provider comparison 入口支持 `--reuse-parser-instances` 候选 warm-state 测量开关；结果会记录 `reuse_parser_instances` 和 `measurement.parser_lifecycle`，默认仍按 `new_per_run` 冷启动口径执行，不改变生产路由。
