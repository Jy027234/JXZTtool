...
- [x] 历史宿主接线与双跑证据归档（仅保留 `archive/` 回溯入口）
- [x] 真实 DOCX 解析与文本解析
- [x] 本地容器运行骨架
- [x] 基础自动化测试
...
- [x] 生产级 PDF 文本解析
- 收口说明：解析栈已落齐 (1) pypdf 段落切分 + 多列页面文本重排，(2) 双通道 pdfplumber 表格识别，(3) 结构项 / 内联结构项 / TOC 条目切分，(4) 表格续行合并 / HIGHLIGHTS 变更日志合并，(5) 重复页眉页脚剥离 + 极短块合并，(6) CID 乱码 / 空白页自动转图走 RapidOCR 兜底，(7) 可选 LLM 边界精修 hook（仅对低置信段落生效），(8) 所有后处理项均可通过 `[[parsers]] options.post_process` 开关。当前 baseline 继续沿用同一工具路径，但门禁入口已拆层：`var/regression/suite.fast.json` 负责日常主线，`suite.full.json` 负责扩展回归，`suite.perf.json` 单独跟踪 27-81-17 与 CMM OCR 两个重样本，避免 layout/OCR 长尾继续把 full 窗口压在贴边状态；69 项单测覆盖含 PDF 各后处理子模块。残差结构差异（历史双跑 raw/展示口径 Block 对位仍有页级块数差）已不再以“向 legacy 收敛”为目标，因为 legacy 在 TOC/表格页常压成单块，继续向其收敛会降质，所以保留当前更细颗粒度的切分作为生产口径。
...
- [x] 队列化 worker
- [x] 历史宿主兼容性资料归档
- 收口说明：相关材料只作为 `archive/` 回溯证据，不再形成主线任务、门禁或灰度计划。
...
## Phase 2: 历史单一宿主接入（已归档）

目标：保留“已验证过宿主嵌入与结果回写边界”的历史结论；具体细节统一以 `archive/` 为准。

- [x] 已完成宿主接入边界、任务入口与结果回写验证
- [x] 已归档历史兼容性证据、双跑资料与差异摘要

说明：主线只保留历史结论，不再维护宿主名称、接口路径、回填字段、双跑报告和辅助脚本等细节。

验收标准：

- [x] 宿主业务接口不被 ParseCore 主线侵入式改造
- [x] 新解析流程可独立重跑
- [x] 历史差异证据可在 `archive/` 中复现
...
- 说明：新增 `POST /parse/batch`（parser-service 兼容根路径）与 `POST /v1/parse/batch`（版本化入口），统一接收 `file_base64 + file_name` 并同步返回 `success / total_pages / pages[] / parser_used / error`，用于对接现有 Node 产品的 parser-service 客户端；页级投影会复用 ParseCore block 元数据输出 `page_number / page_type / text / tables_markdown / confidence`，其中 `toc_entry/lep_entry` 自动归类为 `toc`，`front_matter/revision_record/distribution_list` 自动归类为 `front_matter`，便于以最小改动嵌入现有产品。
...
- [x] 提供历史单一宿主接线资料归档
- 说明：相关替换清单、联调 runbook 与历史验证材料均已移入 `archive/`，此处仅保留归档结论。
...
- [x] Step 1：API 入口可选鉴权。新增 `runtime.api_key_env`，对 `/health` 以外 HTTP 接口支持 `x-api-key` / `Authorization: Bearer` 校验，默认关闭。
- [x] Step 2：告警与运行手册。已把 `/v1/parse/prometheus`、`/v1/parse/metrics`、事件流与建议告警规则收口到 [docs/performance-stability.md](performance-stability.md)。
- [ ] Step 3：perf 口径常态化。仓库内已完成 `perf_tracking` 结构化输出、`--compare-report` 趋势对比、Actions perf 摘要表，以及基于 `PARSECORE_PERF_HISTORY_DIR` 的 runner 持久报告约定；剩余工作是在线上 runner 真正挂载稳定样本环境并开始累计趋势数据。
...
- 不再推进任何单一宿主切流或替换计划；相关资料仅保留在 `archive/` 供历史回看。
...
- [x] P0 手册结构语义前移：DOCX 解析直出 `front_matter / toc_entry / lep_entry / revision_record / distribution_list / body_section / appendix`，不再只给普通 paragraph/table
- [x] P0 DOCX 标题归一化与章节级 chunk 聚合：支持无空格编号 + 中文标题 + 页码尾缀，并让 artifact 角色不进入主 chunk / plain text 视图
- [x] P1 结构可用性指标：`manual_anatomy + structure_quality` 已接入文档快照、index manifest、`tools/regression_baseline.py` drift gate 与 `tools/self_check.py` 摘要
- [x] P1 PDF Manual Anatomy 补齐：在不改 parser 主语义权重的前提下，基于 heading/page_type/table_title 做保守推断，补出 PDF 侧 `body_section / appendix / revision_record / distribution_list / lep_entry` anatomy 视图
...
- regression baseline 现支持 `fixture_relative_path + PARSECORE_REGRESSION_FIXTURE_ROOT` 的跨机器样本解析；在 hosted runner 未挂样本根目录时，CI 会自动降级为 smoke-only / skip slow，而不是把样本缺失误报成代码回归。
