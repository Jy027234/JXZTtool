...
[[parsers]]""",
    ...
    def test_parse_schema_endpoints_expose_frozen_document_contracts(self) -> None:
        ...
        self.assertEqual(ir_schema.json()["x-parsecore"]["schema_version"], "2026-07-ir")
        self.assertEqual(ir_schema.json()["properties"]["projection"], {"const": "ir"})
        self.assertEqual(reader_schema.status_code, 200)
        self.assertEqual(reader_schema.json()["x-parsecore"]["schema_version"], "2026-07-reader")
    ...
    def test_document_projection_structured_exposes_v2_tables_and_quality(self) -> None:
        with TemporaryWorkspace(SAMPLE_CONFIG) as workspace:
            ...
            with TestClient(app) as client:
                ...
                ir_payload = ir.json()
                self.assertEqual(ir_payload["schema_version"], "2026-07-ir")
                self.assertEqual(ir_payload["projection"], "ir")
                self.assertEqual(ir_payload["doc_id"], "doc-table-v2")
                self.assertEqual(ir_payload["source_integrity"]["status"], "verified")
                self.assertEqual(len(ir_payload["source_integrity"]["source_hash"]), 64)
                self.assertTrue(ir_payload["knowledge_unit_diff"]["baseline"])
                self.assertGreaterEqual(len(ir_payload["blocks"]), 1)
                self.assertEqual(len(ir_payload["blocks"][0]["block_fingerprint"]), 64)
                ...
                self.assertEqual(reader_payload["schema_version"], "2026-07-reader")
                ...
                self.assertEqual(coverage_payload["schema_version"], "2026-07-coverage")
